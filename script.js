document.addEventListener("DOMContentLoaded", () => {
    ...
    // ==========================================================================
    // 0. CONTROL DE NAVEGACIÓN Y UTILIDADES GENERALES
    // ==========================================================================
    ...
    // ==========================================================================
    // 1. GUÍA PRÁCTICA PARA INTROVERTIDOS
    // ==========================================================================
        ...
        toolTabs.forEach(btn => {
            btn.addEventListener('click', () => {
                ...
                if (data) {
                    ...
                    setTimeout(() => {
                        if (toolTitle) toolTitle.textContent = data.title;
                        if (toolDesc) toolDesc.textContent = data.desc;
                        if (toolTakeaway) toolTakeaway.textContent = data.takeaway;
    ...
    function renderDilemmas() {
        if (!dilemmasContainer) return;
        const query = dilemmaSearchInput ? dilemmaSearchInput.value.toLowerCase().trim() : "";
    ...
    // ==========================================================================
    // 3. PETICIONES DE ORACIÓN (AUTO-GUARDADO Y BOTÓN + DE 10 EN 10)
    // ==========================================================================
    const initialPrayerRequests = [
        { id: 1, author: "Camila, 18 años", topic: "Paz para mi mente", details: "Tengo semanas sintiéndome abrumada con mis estudios y el miedo a no ser suficiente. Porfa oren para que pueda dormir con tranquilidad.", prayingCount: 28, time: "Hace 40 min" },
        { id: 2, author: "Anónimo", topic: "Sanidad en mi familia", details: "Mis papás están pasando por una crisis pesada y en mi casa solo hay gritos y tensión. Necesito fuerzas para no perder la calma.", prayingCount: 42, time: "Hace 2 horas" }
    ];

    let prayerRequestsList = JSON.parse(localStorage.getItem('prayer_requests_data')) || initialPrayerRequests;
    let visiblePrayersLimit = 10;
    ...
    const loadMorePrayersBox = document.getElementById('load-more-prayers-box');
    const btnLoadMorePrayers = document.getElementById('btn-load-more-prayers');

    window.addPrayerSupport = function(id) {
    ...
    function renderPrayerRequests() {
        if (!prayerRequestsFeed) return;
        prayerRequestsFeed.innerHTML = '';
        
        const itemsToShow = prayerRequestsList.slice(0, visiblePrayersLimit);
            ...
            card.innerHTML = `
                <div>
                    <div class="req-header">
                        <span class="req-topic">${escapeHtml(applyRobloxFilter(req.topic))}</span>
                        <span class="req-time">${req.time}</span>
                    </div>
                    <p class="req-body-text">"${escapeHtml(applyRobloxFilter(req.details))}"</p>
                </div>
                <div class="req-footer">
                    <span class="req-author-tag">— ${escapeHtml(applyRobloxFilter(req.author))}</span>
                    <button type="button" class="btn-pray-counter" onclick="addPrayerSupport(${req.id})">
        ...
        if (loadMorePrayersBox) {
            loadMorePrayersBox.style.display = prayerRequestsList.length > visiblePrayersLimit ? 'flex' : 'none';
        }
    }

    if (btnLoadMorePrayers) {
        btnLoadMorePrayers.addEventListener('click', () => {
            visiblePrayersLimit += 10;
            renderPrayerRequests();
        });
    }
    ...
    // ==========================================================================
    // 4. BIBLIA COMPLETA CANÓNICA (66 LIBROS & NARRACIÓN CONTINUA)
    // ==========================================================================
    const bibleMetadata = {
        antiguo: [
            ...
            { key: "proverbios", name: "Proverbios", chapters: 31, teaching: "Sabiduría practical para el dinero, amistades y decisiones.", takeaway: "El respeto a Dios es el principio de la verdadera inteligencia." },
    ...
    function loadBooks(selectedBookKey = null) {
        if (!bibleTestamentSelect || !bibleBookSelect) return;
        ...
        if (selectedBookKey) {
            bibleBookSelect.value = selectedBookKey;
        }
        loadChapters();
    }

    function loadChapters(selectedChapter = 1) {
        if (!bibleTestamentSelect || !bibleBookSelect || !bibleChapterSelect) return;
        const testament = bibleTestamentSelect.value;
        ...
        bibleChapterSelect.value = selectedChapter;
    ...
    function fetchAndDisplayChapter() {
        ...
        const testament = bibleTestamentSelect.value;
        const bookKey = bibleBookSelect.value;
        const chapterNum = bibleChapterSelect.value || "1";
        const book = (bibleMetadata[testament] || []).find(b => b.key === bookKey);

        if (!book) return;

        if (readingTitleDisplay) readingTitleDisplay.textContent = `${book.name} — Capítulo ${chapterNum}`;
        if (audioBookTitle) audioBookTitle.textContent = `${book.name} ${chapterNum}`;
        if (readingTeachingText) readingTeachingText.textContent = book.teaching;
        if (readingTakeawayText) readingTakeawayText.textContent = book.takeaway;
        ...
        const fallbackText = [
            { num: 1, text: `Este es el pasaje del libro de ${book.name}, capítulo ${chapterNum}.` },
            { num: 2, text: `Aquí el Padre nos enseña que: ${book.teaching}` },
            { num: 3, text: `Recuerda esta verdad para tu corazón: ${book.takeaway}` },
            { num: 4, text: `No temas acercarte con sinceridad; Dios camina contigo en lo cotidiano y su amor permanece firme en todo momento.` }
        ];
    ...
    function renderVerses(versesList) {
        ...
        if (readingTimeDisplay) readingTimeDisplay.textContent = `⏱ ${estMinutes} min de lectura`;
    }

    function getSpokenText() {
        const title = readingTitleDisplay ? readingTitleDisplay.textContent : "";
        const blocks = versesContainer ? versesContainer.querySelectorAll('.verse-block') : [];
        let text = `${title}. `;
        blocks.forEach(b => {
            text += `${b.textContent}. `;
        });
        if (readingTeachingText && readingTakeawayText) {
    ...
    function playBibleSpeech() {
        ...
        isSpeaking = true;
        if (btnAudioPlay) btnAudioPlay.textContent = "⏸ Pausar";
        if (btnAudioStop) btnAudioStop.style.display = "inline-block";
    }
    ...
    function stopBibleSpeech() {
        ...
        isSpeaking = false;
        if (btnAudioPlay) btnAudioPlay.textContent = "▶ Escuchar capítulo";
    ...
    if (bibleTestamentSelect && bibleBookSelect && bibleChapterSelect) {
    ...
    // ==========================================================================
    // 5. TESTIMONIOS Y ANÉCDOTAS (AUTO-GUARDADO Y BOTÓN + DE 10 EN 10)
    // ==========================================================================
    const initialStories = [
        { id: 1, author: "Mateo, 20 años", badge: "De la culpa a la paz", content: "Crecí en una casa donde cada domingo se sentía como un examen moral. Creía que si cometía un error, Dios me tachaba de su lista. Cuando entendí que Él no es un inspector sino un Padre que me acompaña en mis caídas, se me quitó un peso enorme de los hombros." },
        { id: 2, author: "Sofía, 22 años", badge: "Orar sin libreto", content: "Odiaba rezar porque sentía que repetía palabras huecas que no nacían de mí. Dejé de hacerlo por años. Un día, con mucha rabia y tristeza, simplemente le hablé como le hablaría a alguien sentado al lado mío en mi pieza. Por primera vez sentí que me escuchaban de verdad." },
        { id: 3, author: "Lucas, 19 años", badge: "Dudas sin miedo", content: "En mi entorno, hacer preguntas difíciles era pecado. Me alejé pensando que la fe era para mentes ciegas. Pero descubrí al Jesús real: el que debatía con paciencia y no huía de las dudas. Hoy tengo una amistad honesta con Él sin fingir que lo entiendo todo." }
    ];

    let communityStoriesList = JSON.parse(localStorage.getItem('community_stories_feed')) || initialStories;
    let visibleStoriesLimit = 10;
    ...
    const loadMoreStoriesBox = document.getElementById('load-more-stories-box');
    const btnLoadMoreStories = document.getElementById('btn-load-more-stories');

    function renderStoriesFeed() {
        if (!storiesGridFeed) return;
        storiesGridFeed.innerHTML = '';
        
        const itemsToShow = communityStoriesList.slice(0, visibleStoriesLimit);
        ...
        if (loadMoreStoriesBox) {
            loadMoreStoriesBox.style.display = communityStoriesList.length > visibleStoriesLimit ? 'flex' : 'none';
        }
    }

    if (btnLoadMoreStories) {
        btnLoadMoreStories.addEventListener('click', () => {
            visibleStoriesLimit += 10;
            renderStoriesFeed();
        });
    }
    ...
    // ==========================================================================
    // 6. CHAT COMUNITARIO EN VIVO (PERSISTENCIA Y SCROLL AUTOMÁTICO)
    // ==========================================================================
    let currentChatUser = localStorage.getItem('chat_current_user') || null;

    const initialChatMessages = [
        { id: 1, user: "Matías (Chile)", time: "14:10", text: "Hoy rendí una prueba que me tenía sin dormir hace días. Me acordé de que Dios camina en lo cotidiano y respiré hondo. Fuerza a los que tienen una semana pesada." },
        { id: 2, user: "Cami (Colombia)", time: "14:22", text: "Yo no soy de ninguna religión estricta y siempre sentía que no encajaba. Me da mucha paz este rincón donde no te juzgan por tus dudas." },
        { id: 3, user: "Lucas", time: "14:35", text: "Llevo 4 días limpio y dando la pelea. Saber que el Padre no me mira con asco me dio las fuerzas para levantarme de nuevo." }
    ];
    ...
    function checkChatAuth() {
        if (!loginScreen || !activeScreen) return;
        if (currentChatUser) {
            loginScreen.style.display = 'none';
            activeScreen.style.display = 'flex';
            if (currentUserDisplay) currentUserDisplay.textContent = currentChatUser;
            renderChatFeed();
        } else {
            loginScreen.style.display = 'block';
            activeScreen.style.display = 'none';
            if (gateErrorMsg) gateErrorMsg.textContent = '';
    ...
    if (loginForm) {
        loginForm.addEventListener('submit', (e) => {
            ...
            if (containsInappropriateContent(name)) {
                if (gateErrorMsg) gateErrorMsg.textContent = "⚠️ Nombre no permitido. Por favor elige un apodo respetuoso para entrar.";
                usernameInput.value = '';
                return;
            }
    ...
    if (btnLogoutChat) {
        btnLogoutChat.addEventListener('click', (e) => {
            e.preventDefault();
    ...
    // ==========================================================================
    // 7. BANCO DE RESPUESTAS DEL PADRE Y MURO GLOBAL (BOTÓN + DE 10 EN 10)
    // ==========================================================================
    const fatherReplies = {
        pureza: [
    ...
    let currentWallFilter = 'todos';
    let visibleWallLimit = 10;
    ...
    const loadMoreWallBox = document.getElementById('load-more-wall-box');
    const btnLoadMoreWall = document.getElementById('btn-load-more-wall');

    window.supportTestimony = function(id) {
    ...
    function renderTestimonies() {
        if (!testimoniesFeed) return;
        testimoniesFeed.innerHTML = '';

        const filtered = currentWallFilter === 'todos' 
            ? allTestimonies 
            : allTestimonies.filter(item => item.tag === currentWallFilter);

        const itemsToShow = filtered.slice(0, visibleWallLimit);
        ...
        if (itemsToShow.length === 0) {
            ...
            if (loadMoreWallBox) loadMoreWallBox.style.display = 'none';
            return;
        }
        ...
        itemsToShow.forEach((item) => {
            ...
            card.innerHTML = `
                ...
                <div class="testimony-user-side">
                    <p class="testimony-text">"${escapeHtml(applyRobloxFilter(item.text))}"</p>
                ...
                <div class="testimony-footer-bar">
                    <button type="button" class="btn-pray-support" onclick="supportTestimony(${item.id})">
        ...
        if (loadMoreWallBox) {
            loadMoreWallBox.style.display = filtered.length > visibleWallLimit ? 'flex' : 'none';
        }
    }

    if (btnLoadMoreWall) {
        btnLoadMoreWall.addEventListener('click', () => {
            visibleWallLimit += 10;
            renderTestimonies();
        });
    }
    ...
    filterButtons.forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            btn.classList.add('active');
            currentWallFilter = btn.getAttribute('data-filter');
            visibleWallLimit = 10;
            renderTestimonies();
        });
    });
    ...
    // ==========================================================================
    // 8. ORACIONES GUIADAS PARA MOMENTOS REALES & COPIAR
    // ==========================================================================
    ...
    if (prayerButtons && prayerDisplayCard) {
        prayerButtons.forEach(btn => {
            btn.addEventListener('click', () => {
                ...
                if (data) {
                    ...
                    setTimeout(() => {
                        if (prayerScenarioTag) prayerScenarioTag.textContent = data.scenario;
                        if (prayerText) prayerText.textContent = data.text;
                        if (prayerFatherWhisper) prayerFatherWhisper.textContent = data.whisper;
    ...
    // ==========================================================================
    // 9. AUXILIO RÁPIDO PARA LUCHAS DE PUREZA (SOS)
    // ==========================================================================
    const sosData = {
        temptation: {
    ...
    function showSos(type) {
        if (!sosDisplay) return;
        const data = sosData[type];
        if (sosTitle) sosTitle.textContent = data.title;
        if (sosText) sosText.textContent = data.text;
        if (sosQuote) sosQuote.textContent = data.quote;
        sosDisplay.style.display = 'block';
        sosDisplay.scrollIntoView({ behavior: 'smooth', block: 'nearest' });
    }

    if (btnSosTemptation) btnSosTemptation.addEventListener('click', () => showSos('temptation'));
    if (btnSosRelapse) btnSosRelapse.addEventListener('click', () => showSos('relapse'));

    // ==========================================================================
    // 10. BUSCADOR DE CONSUELO POR EMOCIÓN
    // ==========================================================================
    ...
    function findComfort(query) {
        ...
        if (comfortBadge) comfortBadge.textContent = matched.badge;
        if (comfortHeadline) comfortHeadline.textContent = matched.headline;
        if (comfortMessage) comfortMessage.textContent = matched.message;
        if (comfortVerse) comfortVerse.textContent = matched.verse;
    ...
    if (btnSearchComfort) {
        btnSearchComfort.addEventListener('click', () => {
            if (comfortInput) findComfort(comfortInput.value);
        });
    }
    ...
    quickTags.forEach(tag => {
        tag.addEventListener('click', () => {
            ...
            if (comfortInput) comfortInput.value = tag.textContent;
            findComfort(query);
        });
    });

    // ==========================================================================
    // 11. REFLEXIONES DIARIAS, WHATSAPP & SELECTOR DE ÁNIMO
    // ==========================================================================
    const reflexiones = [
    ...
    function updateWhatsappLink(texto) {
        ...
        const mensaje = encodeURIComponent(`${texto}\n\nDescubre más en: https://spider202545.github.io/una-relacion-real/`);
        btnWhatsapp.href = `https://api.whatsapp.com/send?text=${mensaje}`;
    }

    if (quoteText) updateWhatsappLink(quoteText.textContent);

    if (btnQuote && quoteText) {
        ...
        moodButtons.forEach(btn => {
            btn.addEventListener('click', () => {
                ...
                if (data) {
                    ...
                    setTimeout(() => {
                        if (moodTitle) moodTitle.textContent = data.title;
    ...
    // ==========================================================================
    // 12. SONIDO AMBIENTAL ACÚSTICO (WEB AUDIO API)
    // ==========================================================================
    let audioCtx = null;
    ...
    if (btnAmbient) {
        btnAmbient.addEventListener('click', () => {
            if (!isPlayingAmbient) {
                ...
                if (ambientLabel) ambientLabel.textContent = "Calma activa";
                isPlayingAmbient = true;
            } else {
                clearInterval(ambientInterval);
                btnAmbient.classList.remove('playing');
                if (ambientLabel) ambientLabel.textContent = "Música / Calma";
